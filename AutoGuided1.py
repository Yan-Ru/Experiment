    ...
    def Sent(self):
        while True:
            if self.Copter0Home[0] != None and self.Copter1Home[0] != None and self.Copter2Home[0] != None:
                ser.write(b'1,' + str(Home.lat).encode('utf-8') + b','
...
Path = next((path for path in paths if path[1].lat == AssignRoute[1][-1].lat and path[1].lon == AssignRoute[1][-1].lon))
Path = AssignRoute[1]+Path[2:]
Path[-1] = Path[0]
